...
import { useNavigate } from "react-router-dom";
    ...
    export default function ProductPicturesInterface() {
  ...
  const fillProducts=()=>{
    ...
    // fill productList
   const fecthAllProductsList=async(pid)=>{
    var result = await postData('productslist/productslist_item_by_productid', {productid:pid})
    setProductsListItem(result.data)
   }

   const fillProductsList=()=>{
    return productsListItem.map((item)=>{
      return <MenuItem value={item.productslistid}>{item.productslistname}</MenuItem>
    })
   }
   ...
   const handleProductsChange=(event)=>{
    ...
    fecthAllProductsList(event.target.value)
   }
    ...
    const handleClick = async ()=>{
        if(validation()){
            ...
            var result = await postData("productpictures/productpictures_submit", formData);
          console.log(error)
             console.log(result) 
           ...
           return(
            <div className={classes.container}> 
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12}>
                    <Grid className={classes.heading}> Add New Product Picture</Grid>
                    </Grid>
                    <Grid item xs={6}>
