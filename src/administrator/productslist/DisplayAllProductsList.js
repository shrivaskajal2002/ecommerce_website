...
export default function DisplayAllProducts() {
  ...
  const handleEditData = async () => {
    if (validation()) {
      var body={categoryid:categoryId,subcategoryId:subcategoryId,status:status}

      var result = await postData("products/productsItem", body);
      console.log(error);
      console.log(result);
    ...
    return (
      <MaterialTable
        ...
        actions={[
          ...
          isFreeAction:true,
          onClick:(event)=> navigate("/dashboardInterface/ProductsListInterface")
          }
        ]}
      />
    );
